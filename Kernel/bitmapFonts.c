...
// Faaa que es esta super matriz? Es la codificación en un bitmap de cada letra.  
// El 0 es el color del fondo, el 1 es el color del caracter. 
// A partir del espacio, de codigo ascii 32, se puede obtener la codificación gráfica de cada letra
// Segun la formula letters[x-' '], donde x es el ascii del caracter a dibujar
// La dimensión de cada bitmap es de 11x8, resolucion mas que suficiente para este tp 
// Cada bit del bitmap puede representar mas de un pixel. Eso dependera de quien llame a este header y el font size.
static unsigned char letters[][6] = {
...
unsigned char * getBitmapFor( char c ) {
    if (c<' ') return 0; 
    return letters[c-' ']; 
}
